...
func _ready() -> void:
	...
	var scoreLeftLabel = $ScoreLeft
	var scoreRightLabel = $ScoreRight
	
	Signals.playerScored.connect(pointEarned)
...
func pointEarned(point: int, player: int) -> void:
	...
	if player == 1:
		var scoreLabel = $ScoreLeft # Get reference to ScoreLeft label
		if scoreLabel:
			print("changing p1")
		...
		var scoreLabel = $ScoreRight # Get reference to ScoreRight label
		if scoreLabel:
			print("changing p2")
			var currentScore = int(scoreLabel.text) # Convert current text to integer
			currentScore += point                  # Add points
